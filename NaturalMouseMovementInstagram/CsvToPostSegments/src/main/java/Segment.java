...
public class Segment {
    ...
    public static void writeAnalyticsToCsv(ArrayList<Segment> segments){
        ...
        try (CSVWriter writer = new CSVWriter(new FileWriter(segmentCsvFile))) {
            // Write header
            String[] header = {"numOfPoints", "duration", "displacement", "directionVector_x", "directionVector_y","segmentStartPoint_x","segmentStartPoint_y","segmentEndPoint_x","segmentEndPoint_y"};
            ...
            for(Segment segment : segments) {
                ...
                row.add(String.valueOf(segment.segmentPoints.getFirst().get(0))); // segment start point x val
                row.add(String.valueOf(segment.segmentPoints.getFirst().get(1))); // segment start point y val
                row.add(String.valueOf(segment.segmentPoints.getLast().get(1))); // segment end point y val
                row.add(String.valueOf(segment.segmentPoints.getLast().get(1))); // segment end point y val
                data.add(row);
            }
